<template>
  <div class="table-box">
    ...
    <myDialog :title="detailParams.title" ref="myDialog1" draggable width="80%" :before-close="beforeClose1">
      <template #content>
        <addEdit
          v-if="IsShowAdd"
          ref="addEditRef"
...
import { task_List, task_delete } from "@/api/sportCheck/task";
import addEdit from "@/views/sportCheck/task/components/addEdit.vue";
...
// 默认不做操作就直接在 ProTable 组件上绑定	:requestApi="task_List"
const getTableList = (params: any) => {
  let newParams = JSON.parse(JSON.stringify(params));
  return task_List(newParams);
};
// 枚举值
...
const deleteAccount = async (params: any) => {
  await useHandleData(task_delete, { id: params.id }, `删除【${params.name}】`);
  proTable.value?.getTableList();
};
